...
// boxsize: actual size of the client's displayed viewbox
export default function getTransform(scale: number, w: number, h: number, boxSize: number): string {
    if(scale === 1) {
        return `translate(${(boxSize - (w * scale)) / 2} ${(boxSize - h * scale) / 2})`
      } else {
